...
- **Render autodeploy is PER SERVICE.** ⚠️ Corrected 2026-07-26 04:20: the **API** (`srv-chgb1k67avjbbju8aoig`) autodeploys on push — verified, a push at ~03:00 was live by 03:11 with nobody asking for it. The **game** (`srv-d9ebcvtaeets73ar91sg`) does not; it needs the CLI (`render` v2.5.0, `/opt/homebrew/bin/render`): `render deploys create srv-d9ebcvtaeets73ar91sg --confirm` (game) or `srv-chgb1k67avjbbju8aoig` (api = server.pikme.tv). So: pushing the GAME changes nothing until the CLI runs; pushing the API ships it immediately. Anything stating one blanket rule for both services is stale.
