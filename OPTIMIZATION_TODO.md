...
> sharper, smoother, more reactive" / fps / WebRTC — START HERE.** This is the single
...
> fps, WebRTC, UDP, transport, prediction, reconciliation, interpolation, lag compensation.
>
> ⛔ **120Hz is OFF the table** (user decision, 2026-07-25) — the sim stays 60Hz. It used to be item 1
> here. Do not re-add it, and do not propose raising TICK_RATE as a latency win.
>
...
### [ ] 1. On-device feel-check the reconciliation  · priority: HIGH · effort: S
...
### [ ] 2. Server per-tick input FIFO queue + ack  · priority: MED · effort: M
...
### [ ] 3. ★ WebRTC / UDP transport  · priority: HIGH (biggest real gap) · effort: L
...
### [ ] 4. Lag compensation (server rewind)  · priority: MED · effort: M–H
...
### [ ] 5. Render-perf batch (sustain a steady 60fps on device)  · priority: MED · effort: M–L
...
### [ ] 6. Assets fully on-device  · priority: LOW · effort: M→L
...
- WebGL / react-native-skia — the only path to GPU sprite headroom beyond the canvas renderer (big rewrite).
...
- Brawl Stars netcode comparison (this session): we're 60/60/60Hz — at/above BS's cited ~20–30Hz (unofficial); behind only on transport (they're UDP) + lag-comp. So the transport (#3) and lag-comp (#4) items are the substance; raising the tick rate is not on the table.
