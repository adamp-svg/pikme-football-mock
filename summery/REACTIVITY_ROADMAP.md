...
- **ProMotion 120Hz.** ⛔ **DROPPED by the user (2026-07-25) — do not plan work around this.** *Would need:* a native CADisplayLink shim (WebKit private API — App Store review risk) or the Skia path. Only meaningful once the NOW per-frame CPU cost is already low. *Unlocks:* 120fps + lower input-to-photon latency.
