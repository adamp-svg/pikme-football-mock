...
- **⚠️ PUSHING EITHER REPO IS A PRODUCTION DEPLOY. Both services autodeploy — verified 2026-07-26 17:1x.** The **API** `srv-chgb1k67avjbbju8aoig` was already known to. The **game** `srv-d9ebcvtaeets73ar91sg` was documented here as having a dead webhook that needed the CLI, **and that is now false**: after a push of `football-mock` today, `curl -s https://pikme-football.onrender.com/shared/bot-ai.js | shasum -a 256` came back **byte-identical to local HEAD** with no CLI deploy run by anyone. Assume a push is live in ~1-2 minutes, on the phone too (TestFlight loads `PROD_GAME_URL`). And note this repo is pushed **many times a day as a matter of course** — 117 push entries over the eight days to 2026-07-26 — so a commit typically reaches prod within the hour whether or not the agent that wrote it pushes. **The decision point is the COMMIT.** Verify with that same curl+shasum, not by trusting this line. The manual CLI is still the fallback if a webhook is genuinely down: `render deploys create srv-d9ebcvtaeets73ar91sg --confirm` (game) / `srv-chgb1k67avjbbju8aoig` (api).
  **Why this matters more than it looks:** the old wording made a game-repo push read as harmless-until-you-run-the-CLI. It is not. `git push` on either repo needs an explicit "push"/"deploy" in the user's message, and there is no second confirmation step after it.
