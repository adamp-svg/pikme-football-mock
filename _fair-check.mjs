...
ok(Math.max(...hs) <= 542, `nobody sees more than the 540-unit window (max ${Math.max(...hs)})`);
ok(spread <= 2, `every device sees the same pitch height (spread ${spread} units across ${hs.length} devices: ${hs.join(', ')})`);
...
ok(seen.filter(s => s.d.h < 500).every(s => s.v.bandY <= 4), `phones carry at most a rounding sliver (${seen.filter(s => s.d.h < 500).map(s => s.v.bandY).join(', ')} art px)`);
console.log(fails ? `\n❌ ${fails} FAILED` : '\n✅ the cap holds');
